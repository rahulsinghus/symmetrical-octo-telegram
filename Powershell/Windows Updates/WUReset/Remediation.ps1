...
Remove-Item $env:systemroot\SoftwareDistribution.old -force -recurse -verbose -ErrorAction SilentlyContinue
Remove-Item $env:systemroot\SoftwareDistribution.bak -force -recurse -verbose -ErrorAction SilentlyContinue
...
} while ($status.Status -eq "Stopping")
