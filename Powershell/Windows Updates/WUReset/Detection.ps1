...
Write-Log -File $LogFile -Status Information -Text "....Forcing discovery StartInteractiveScan...";
C:\Windows\System32\UsoClient.exe StartInteractiveScan
Write-Log -File $LogFile -Status Information -Text "....Forcing discovery ScanInstallWait...";
C:\Windows\System32\UsoClient.exe ScanInstallWait
Write-Log -File $LogFile -Status Information -Text "....Forcing discovery StartInstall...";
C:\Windows\System32\UsoClient.exe startinstall
Write-Log -File $LogFile -Status Information -Text "....Forcing discovery Resume on Restart...";
...
if ($BuildNumber.BuildNumber -lt "19042") {
    Write-Log -File $LogFile -Status Information -Text "Non-Compliant"
    write-host "Not Compliant" $BuildNumber.Version
    exit 1
}else {
    Write-Host "Build Number Greater than or equal to 19042"
}
Write-Log -File $LogFile -Status Information -Text $BuildNumber.BuildNumber

$ReleaseID  = $BuildVersions | where BuildNumber -eq (Get-WMIObject win32_operatingsystem).BuildNumber
    ...
    If ($searchresult.Updates.Count -gt 0) { 
        ...
        For ($i=0; $i -lt $Count; $i++) { 
            ...
            If ($update.IsDownLoaded -eq "True") {  
                ...
                $temp = "" | Select KB,IsDownloaded,Title, Computer 
                $temp.Computer = $c 
